...
def test_logger():
    ...
    try:
        m = mylogger.get_logger('test', '/root/data_architecture/log')
        m.debug('hi, debug')
...
def test_config():
    ...
    try:
        m = myconfig.get_config('/root/data_architecture/share/test.config')
...
def test_login(db):
    ...
    col_user = db['User']
    doc_user = user.check_passwd(col_user, 'iam2', 'biggong')
    if not doc_user:
        return False
